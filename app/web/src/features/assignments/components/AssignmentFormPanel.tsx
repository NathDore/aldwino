...
import { useAssignmentForm, type AssignmentFormIntent } from "../hooks/useAssignmentForm";
...
export function AssignmentFormPanel({ onClose, assignmentToEdit, intent = "edit" }: AssignmentFormPanelProps) {
...
function AssignmentEditForm({
  ...
  const { formState, updateField, handleSubmit, isLoading, isRescheduling, todayDateInput } = useAssignmentForm(
    assignmentToEdit,
    onClose,
    intent,
  );
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="space-y-4">
        ...
        {isRescheduling ? (
          <div>
            <p className={`block ${LABEL_FONT_SIZE} font-semibold text-slate-700 mb-1.5`}>Assignment</p>
            <p className="text-base whitespace-normal break-words text-slate-900">{assignmentToEdit.name}</p>
          </div>
        ) : (
          <>
            <div>
      ...
      <div className="flex justify-end gap-2 mt-auto pt-4">
        ...
        <Button
          ...
          disabled={isLoading || (!isRescheduling && courses.length === 0)}
        >
          {isLoading ? "Saving..." : isRescheduling ? "Reschedule" : "Save"}
        </Button>
      </div>
    </div>
  );
