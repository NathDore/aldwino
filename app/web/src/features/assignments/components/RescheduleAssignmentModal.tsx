...
import { RescheduleAssignmentForm } from "./RescheduleAssignmentForm";
...
export function RescheduleAssignmentModal({ item, onClose }: RescheduleAssignmentModalProps) {
  const { course } = item;
      ...
      >
        <div className="flex items-start justify-between gap-2 px-10 py-3 border-b border-slate-200 shrink-0">
          <div className="min-w-0 flex items-center gap-2">
            ...
            <p className="text-sm font-bold text-slate-900 shrink-0">Reschedule Assignment</p>
          </div>
        ...
        <div className="px-10 py-4 overflow-hidden min-h-0">
          <RescheduleAssignmentForm item={item} onClose={handleClose} />
        </div>
      </div>
    </div>,
    document.body
