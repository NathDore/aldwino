...
import { ASSIGNMENT_ROW_GRID, ASSIGNMENT_ROW_HEIGHT } from "../utils/assignmentRowLayout";
...
export function AssignmentRow({ assignment, course, actions, onReschedule, onEdit, onDelete }: AssignmentRowProps) {
  ...
  return (
    <div className={`${ASSIGNMENT_ROW_GRID} ${ASSIGNMENT_ROW_HEIGHT} items-center hover:bg-slate-50`}>
      <div className="px-1.5 text-xs font-medium text-slate-900 truncate">{assignment.name}</div>
      <div className="px-1.5 text-xs text-slate-700 min-w-0">
      ...
      <div className="px-1.5 text-xs text-slate-600">{formatAssignmentDueDate(assignment.dueDate)}</div>
